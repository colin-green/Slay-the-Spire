...
class Character {
	constructor() {
		...
		this.cardsPerTurn = 5;
		this.deck = [
			findCardRelicEnemy(cards.colorlessCardPool, 'Strike'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Strike'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Strike'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Strike'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Defend'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Defend'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Defend'),
			findCardRelicEnemy(cards.colorlessCardPool, 'Defend'),
			findCardRelicEnemy(cards.cursePool, "Ascender's Bane")
		];
		this.relics = [];
		this.potions = [];
		this.block = 0;
...
function createStarterDeck() {
	switch (character.name) {
		case 'Ironclad':
			...
			character.relics.push(
				findCardRelicEnemy(relics.starterRelics, 'Burning Blood')
			);
			break;
		...
		case 'Silent':
			...
			character.deck.push(
				findCardRelicEnemy(cards.silentCardPool, 'Neutralize')
			);
			character.relics.push(
				findCardRelicEnemy(relics.starterRelics, 'Ring of the Serpent')
			);
			break;
		...
		case 'Defect':
			...
			character.relics.push(
				findCardRelicEnemy(relics.starterRelics, 'Cracked Core')
			);
			break;

		case 'Watcher':
			character.deck.push(
				findCardRelicEnemy(cards.watcherCardPool, 'Vigilance')
			);
			character.deck.push(
				findCardRelicEnemy(cards.watcherCardPool, 'Eruption')
			);
			character.relics.push(
				findCardRelicEnemy(relics.starterRelics, 'Pure Water')
			);
			break;
...
function showDrawPile() {
	...
	if (drawPile.length === 0) {
		...
		let sortedDrawPile = drawPile.slice();
		sortedDrawPile.sort(function(a, b) {
			var x = a.name;
			var y = b.name;
			if (x < y) {
				return -1;
		...
		sortedDrawPile.forEach(card => console.log(card.name));
	}
	console.log('\n');
}
...
function whaleBonus() {
	let option12choices = [
		`+Max HP`,
		'Enemies in the next 3 combats will have 1 HP',
		'Remove a card',
		'Transform a card',
		'Upgrade a card',
		'Choose a card to obtain',
		'Obtain a random rare card/uncommon colorless card/common relic',
		'Recieve 100 gold',
		'Obtain 3 random potions'
	];
	let option3disadvantages = [
		'Lose max HP',
		'Take damage',
		'Obtain a Curse',
		'Lose all gold'
	...
	let option3disadvantage =
		option3disadvantages[
			Math.floor(Math.random() * option3disadvantages.length)
		];
	...
	let option3reward =
		option3rewards[Math.floor(Math.random() * option3rewards.length)];
	...
	if (hallwayCounter < 3) {
		...
		switch (fight) {
			...
			case 'Small Slimes':
				...
				if (roll === 0) {
					combatEnemies = [
						findCardRelicEnemy(enemies.act1Enemies, 'Spike Slime (M)'),
						findCardRelicEnemy(enemies.act1Enemies, 'Acid Slime (S)')
					];
				} else {
					combatEnemies = [
						findCardRelicEnemy(enemies.act1Enemies, 'Acid Slime (M)'),
						findCardRelicEnemy(enemies.act1Enemies, 'Spike Slime (S)')
					];
				}
				break;
			default:
